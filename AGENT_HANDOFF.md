...
> 生成时间：2026-08-04。基线线上 commit：`7d291b9`（之后已落地"实底/透明双目录"方案，见第七节）。
...
> **生成阶段策略（已与用户确认）**：允许直接生成**实底图**入库，不必强求生成时透明——ImageGen 实测 `background:transparent` 后端不生效，仍输出实底方块图。实底图游戏能正常加载（只是方块衬底，**不是破图**），去实底统一留到本阶段**批量处理**。**不要因为图是实底的就删除重生成**；统一去实底时跑 `rembg` 即可（零 ImageGen 额度）。

- 怪图背景是 AI 渲染出的场景底（**非纯色**）。Pillow 色键会留一圈底色、还会啃到怪身，必须用 ML 抠图（`rembg`）。

**实底(图鉴) / 透明(战斗) 双目录方案（已落地，务必遵守）：**

- `assets/monsters/{key}.png` = **实底方块**，供图鉴使用（`monsterSVG(def,px)`，不传 transparent）。已压缩到 512px 全彩 RGB（保真档），60 张约 21.5MB。
- `assets/monsters_alpha/{key}.png` = **透明底**，供战斗使用（`monsterSVG(def,px,true)`）。由同一张源图经 `rembg` 抠图 + 缩到 384px 生成，60 张约 5.5MB。
- 两个目录**同名同 key**，靠文件夹区分，绝不重名冲突；代码按 `transparent` 参数决定读哪个目录。
- `verify_art.py` **只校验 `assets/monsters/`**；`assets/monsters_alpha/` 镜像相同 key。新增/补画怪物时，**两个目录都要产出对应文件**（否则战斗找不到透明版 → 回落实底 → 再回落 SVG）。
- 玩家立绘 `assets/player/player.png` 已就地抠透明（只用于角色面板 + 战斗，不在图鉴）。职业 5 张 `assets/origins/*.png` 仍是仓库旧版实底（图鉴不用、建立角色界面显示）。
...
1. 怪物图双目录方案已落地：图1–6 共 60 只怪既有实底（图鉴）又有透明（战斗）；图7–10 共 40 只仍回落 SVG，待补图 + 在 `assets/monsters/` 与 `assets/monsters_alpha/` 两目录各产出版本。
2. `fade_moth` 按数据生成为蜘蛛状（`body:arachnid`）；若想真像蛾，需改 `index.html` 的 art 字段（属数据改动，先确认）。
3. 职业 5 张 `assets/origins/*.png` 仍仓库旧版实底（图鉴不用、建立角色界面显示）；`assets/player/player.png` 已抠透明供战斗/角色面板。
...
- ❌ **不要因为生成出的图是实底就删除重生成**；实底图先正常入库（游戏能加载，只是方块底），去实底统一在 rembg 批处理阶段做。
